...
// let scene, camera, renderer, analyser, dataArray, bars, barBorders;
// let audioElement;
// const bins = 32;
...
//   // renderer.domElement.style.mixBlendMode = 'difference'; // Apply mix-blend-mode to canvas
//   // document.body.appendChild(renderer.domElement);

//   // Audio setup
//   audioElement = new Audio('/audio/2.mp3');
...
//   const barMaxHeight = window.innerHeight * 0.7; // Set max height to 60% of window height
//   for (let i = 0; i < bins; i++) {
//     // Create bar
//     const geometry = new THREE.PlaneGeometry(barWidth, 1);
//     // const material = new THREE.MeshBasicMaterial({ color: 0x808080 });
//     const material = new THREE.MeshBasicMaterial({ color: "#FF7722" });
...
//     border.position.set(i * barWidth - window.innerWidth / 2 + barWidth / 2, -window.innerHeight / 2 + 0.5, 1); // Slightly above to avoid z-fighting
//     scene.add(border);
...
//   // Update bars
//   const barMaxHeight = window.innerHeight * 0.7; // Use 60% of window height
//   bars.forEach((bar, i) => {
...
//     uniform float uAmplitude;
//     void main() {
//       vec3 colorStart = vec3(1.0, 0.467, 0.133); // #FF7722 (normalized)
//       vec3 colorEnd = vec3(1.0, 0.0, 0.0);      // Bright yellow
//       vec3 color = mix(colorStart, colorEnd, uAmplitude);
...
function init() {
  ...
  // Audio setup
  audioElement = new Audio('/audio/shiv.mp3');
  const context = new (window.AudioContext)();
  const source = context.createMediaElementSource(audioElement);
  analyser = context.createAnalyser();
  analyser.fftSize = bins * 2;
  ...
  for (let i = 0; i < bins; i++) {
    ...
    // Create border for the bar
    const borderGeometry = new THREE.EdgesGeometry(new THREE.PlaneGeometry(barWidth, 1));
    const borderMaterial = new THREE.LineBasicMaterial({ color: "white" });
    const border = new THREE.LineSegments(borderGeometry, borderMaterial);
    border.position.set(i * barWidth - window.innerWidth / 2 + barWidth / 2, -window.innerHeight / 2 + 0.5, 1);
    scene.add(border);
    barBorders.push(border);
